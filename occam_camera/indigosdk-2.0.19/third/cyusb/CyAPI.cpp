    ...
    if (hwDeviceInfo != INVALID_HANDLE_VALUE) {
        ...
        if (SetupDiEnumDeviceInterfaces ( hwDeviceInfo, 0, (LPGUID) &DrvGuid,
            deviceNumber, &devInterfaceData)) {
                ...
                // particular device.
                SetupDiGetDeviceInterfaceDetail ( hwDeviceInfo, &devInterfaceData, NULL, 0,
                    &requiredLength, NULL);
...
bool CCyUSBDevice::Open(UCHAR dev){
    ...
    if (CreateHandle(dev)) {
        ...
        if ((BcdUSB & BCDUSBJJMASK) == USB30MAJORVER)
        {
            if(GetInternalBosDescriptor()) // USB3.0 specific descriptor
			{
				...
				catch(char *Str)
				{
					...
					MessageBox(NULL,"Please correct firmware BOS descriptor table","Wrong BOS Descriptor",0);
        ...
        for (int i=0; i<Configs; i++) 
		{
			...
			{
				char *Str1 = Str; // just to ignore warning
				MessageBox(NULL,"Please correct firmware descriptor table","Wrong Device Configuration",0);
				Close(); // Close the device handle.
				return false;
			}
        }
